      ...
      securestore.getItemAsync('details').then(token=>{
        ...
        if (token){
         let userToken=JSON.parse(token)
          ...
          fetch(`https://eathub-go.herokuapp.com/vendor/pending_items/`,requestOptions).then(response=>response.json()).then(result=> {
    ...
    securestore.getItemAsync('details').then(token=>{
      ...
      if (token){
       let userToken=JSON.parse(token)
        ...
        console.log(userToken)
        fetch(`https://eathub-go.herokuapp.com/vendor/delete_from_cart/${id}/`,requestOptions).then(response=>response.json()).then(result=> {
        //setsection(previousarr=>[...previousarr,...parsedresults])
          setrefresh(false)
          console.log(result)
          makeInquiry()
  ...
  function makeInquiry(){
    ...
    if(items!==0){return (
      <View style={styles.container}>
        ...
        <LottieView source={require('../assets/reading-lady.json')}
            autoPlay={true}
            loop={true}
        ...
        ></LottieView>
          ...
          <View style={{flexDirection:'row',justifyContent:'space-between',marginTop:15,marginBottom:10,paddingHorizontal:15}}>
           <View style={{flexDirection:'row'}}>
            ...
            <TouchableOpacity>
              <Text style={styles.appName}>Pending items</Text>
            </TouchableOpacity>
          </View>
      <Animated.FlatList 
        data={items}
          ...
          return <Animated.View style={{flexDirection:'row',
            ...
            <View style={{flex:2}}>
               <View style={{flexDirection:'row',justifyContent:'space-between',alignItems:'center'}}>    
                  ...
                  <TouchableOpacity  onPress={()=>delete_from_cart(item.id)}>
             ...
             <View  style={{flexDirection:"row",justifyContent:"space-between",marginTop:10,alignItems:"center"}}>
               <View style={{flexDirection:"row",justifyContent:"space-between",alignItems:"center"}}>
                  ...
                  <TouchableOpacity style={[styles.continue,styles.center,{marginTop:5}]} onPress={()=>addToCart(item.title,item.id)}>
                      {arr.map((index)=>{
                          return(
                            <MotiView 
